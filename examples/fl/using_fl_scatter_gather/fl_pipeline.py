...
from examples.fl._helper import aggregator
...
def scatter_gather_iteration(
    ...
    def scatter_gather_iter(**kwargs):
        ...
        for input_name, input_obj in gather_instance.inputs.items():
        ...
        aggregate_model = aggregator(aggregated_model=gather_instance.outputs.aggregated_output)
        aggregate_model.inputs.aggregated_model.mode = "eval_mount"
        aggregate_model.outputs.final_model.mode = "rw_mount"
        aggregate_model.compute = gather_strategy["compute"]
        ...
        return aggregate_model.outputs
    ...
    def fl_pipeline():
        ...
        for i in range(0, iterations):
            ...
            iteration_body.outputs.final_model = Output(
                type=AssetTypes.URI_FOLDER,
                mode="mount",
                path=custom_fl_data_path(
                    gather_strategy["datastore"],
